	...
	Chart.controllers.financial = Chart.DatasetController.extend({
		...
		updateElement: function(candle, index, reset) {
			...
			candle._model = {
				...
				label: dataset.data[index].label, // to get label value please use dataset.data[index].label
		...
		updateElementGeometry: function(rectangle, index, reset) {
			...
			model.base = reset ? base : (candle.c + candle.o) / 2;
			model.x = ipixels.center;
			model.y = reset ? base : (candle.c + candle.o) / 2;
			model.height = undefined;
			model.width = ipixels.size / 2;
			model.candle = candle;
		...
		calculateCandleValuesPixels: function(datasetIndex, index) {
			...
			return {
				...
				barra: datasets[datasetIndex].data[index].barra
			};
		},
